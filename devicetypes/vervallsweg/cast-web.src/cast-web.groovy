...
def speak(phrase) {
    ...
    if( phrase.contains(":") )
    {
    	def newPhrase = phrase.split(":")
        return playTrackTts( newPhrase[0], newPhrase[1] )
    }
    else
...
def playTrackTts( provider, phrase ) {
    logger('info', "provider:" + provider + ", phrase:"+ phrase)
    
    return setMediaPlaybackShortOther('audio/mp3', provider, phrase, 'BUFFERED', 'SmartThings', 'SmartThings playback', 'https://lh3.googleusercontent.com/nQBLtHKqZycERjdjMGulMLMLDoPXnrZKYoJ8ijaVs8tDD6cypInQRtxgngk9SAXHkA=w300')
}
...
def setMediaPlaybackShortOther(mediaType, provider, phrase, mediaStreamType, mediaTitle, mediaSubtitle, mediaImageUrl) {
    sendEvent(name: "status", value: "playing", changed: true)
    logger('debug', "Executing 'setMediaPlaybackShortOther'; mediaType: "+mediaType+" provider: "+provider+" phrase:"+phrase+" mediaStreamType: "+mediaStreamType+" mediaTitle: "+mediaTitle+" mediaSubtitle: "+mediaSubtitle+" mediaImageUrl: "+mediaImageUrl)
    if( provider == "Naver" ) {
    	sendHttpRequest(getDataValue('apiHost'), '/setMediaPlaybackNaver?address='+getDataValue('deviceAddress')+'&mediaType='+mediaType+'&ttsText='+urlEncode(phrase)+'&mediaStreamType='+mediaStreamType+'&mediaTitle='+urlEncode(mediaTitle)+'&mediaSubtitle='+urlEncode(mediaSubtitle)+'&mediaImageUrl='+mediaImageUrl)
    }
}
